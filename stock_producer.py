...
REALTIME_INTERVAL = 60  # seconds per fetch
        ...
        while True:
            ...
            for symbol in symbols:
                try:
                    ...
                    open_price = float(daily_row["Open"])  # Giá mở phiên từ daily data
                    high_price = float(daily_row["High"])
                    low_price = float(daily_row["Low"])
                    close_price = float(minute_row["Close"])  # Giá đóng phút realtime
